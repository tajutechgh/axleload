...
<div class="row pt-2 pb-2">
    <div class="col-sm-9">
		<h4 class="page-title">Manage Over Load Cases</h4>
    </div>
</div><!-- End Breadcrumb-->
		            ...
		            <table id="default-datatable" class="table table-bordered">
		                ...
		                <tbody>
		                	@foreach ($excessweights as $excess)
		                	    @if ($excess->overload != 1)
		                	    	<tr>
		                	    	    <td>{{$excess->station['station_name']}}</td>
		                	    	    <td>{{$excess->vehicle_number}}</td>
		                	    	    <td>{{$excess->gross_excess_weight}}</td>
		                	    	    <td>{{$excess->fine_amount}}</td>
		                	    	    <td>
		                	    	    	{{-- payment button --}}
		                	    	        <a href="{{ route('show.excess',$excess->id) }}" class="btn btn-success fa fa-money" title="Click here to pay fine"> Pay Fine</a>
		                	    	    </td>
		                	    	</tr>
		                	    @endif
		                	@endforeach
